...
function loadProfileImages() {
  ...
  E.profilePicture.src = `${location.origin}${location.pathname}${profileMatch.path}`;
  const bannerMatch = IMAGES.find(img => img.id === 'banner-image');
  E.bannerImage.src = `${location.origin}${location.pathname}${bannerMatch.path}`;
}
  ...
  toc.forEach((x, i, a) => {
    ...
    const sortedImages = IMAGES
        ...
        .forEach(img => {
          ...
          viewImage.src = `${location.origin}${location.pathname}${img.path}`;
...
document.addEventListener('click', e => {
  ...
  if (e.target.classList.contains('view')) {
    ...
    viewImage.src = `${location.origin}${location.pathname}${imageData.path}`;
